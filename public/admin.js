...
async function checkTokenLogin() {
    ...
    if (hash && hash.startsWith('#token=')) {
        ...
        try {
            // Validate token server-side via RPC
            const { data, error } = await supabaseClient.rpc('validate_site_token', { p_token: token });

            if (error || !data) {
                console.error('Token validation failed:', error);
                showToast('Invalid or expired login link. Please use the portal.', 'error');
                return;
            }

            // RPC returns site_key — verify it matches this site
            if (data !== CONFIG.SITE_KEY) {
                console.error('Token belongs to different site');
                showToast('Invalid login link for this site.', 'error');
                return;
            }

            // Token is valid and matches this site
            sessionToken = token;
            sessionStorage.setItem('session_token', sessionToken);
            sessionStorage.setItem('authenticated', 'true');
            supabaseClient = window.supabase.createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_ANON_KEY, {
            ...
            showToast('Login failed. Please try again from the portal.', 'error');
        }
    }
}
...
function loadGBPGuide() {
    ...
    // Detect business category heuristic
    var category = 'Local Business';
    if (CONFIG.SCHEMA && Array.isArray(CONFIG.SCHEMA)) {
        var sectionIds = CONFIG.SCHEMA.map(function(s) { return s.id; });
        var allFieldIds = [];
        CONFIG.SCHEMA.forEach(function(s) {
